...
describe('investmentPolicyEngine', () => {
  ...
  it('warns when allocation drift exceeds the rebalancing threshold', () => {
    ...
    const warning = evaluation.warnings.find(
      (policyWarning) => policyWarning.ruleId === 'rebalancing-threshold' && policyWarning.actualWeight === 0.8,
    )
  ...
  it('preserves philosophy text', () => {
    const philosophyText = 'Buy quality assets when valuation and durability align.'
    const portfolio = createPortfolio([
      createHolding({
        id: 'global-index',
        name: 'Global Index',
    ...
    expect(evaluation.policyId).toBe('long-term-policy')
    expect(evaluation).not.toHaveProperty('investmentPolicy')
    expect(createPolicy({ philosophy: { text: philosophyText } }).philosophy.text).toBe(philosophyText)
  })
